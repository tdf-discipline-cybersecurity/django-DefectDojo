...
class SslLabsParser(Parser):
        ...
        for host in data:
            ...
            for endpoints in ssl_endpoints:
                ...
                if "vulnBeast" in endpoints["details"]:
                    description += (
                        "vulnBeast: "
                        + str(endpoints["details"]["vulnBeast"])
                        + "\n"
                    )
                ...
                if "stsPreload" in endpoints["details"]:
                    description += (
                        "stsPreload: "
                        + str(endpoints["details"]["stsPreload"])
                        + "\n"
                    )
                ...
                if "heartbleed" in endpoints["details"]:
                    description += (
                        "heartbleed: "
                        + str(endpoints["details"]["heartbleed"])
                        + "\n"
                    )
                if "heartbeat" in endpoints["details"]:
                    description += (
                        "heartbeat: "
                        + str(endpoints["details"]["heartbeat"])
                        + "\n"
                    )
                if "openSslCcs" in endpoints["details"]:
                    description += (
                        "openSslCcs: "
                        + str(endpoints["details"]["openSslCcs"])
                        + "\n"
                    )
                ...
                if "poodleTls" in endpoints["details"]:
                    description += (
                        "poodleTls: "
                        + str(endpoints["details"]["poodleTls"])
                        + "\n"
                    )
                ...
                if "freak" in endpoints["details"]:
                    description += (
                        "freak: " + str(endpoints["details"]["freak"]) + "\n"
                    )
    ...
    def suite_data(self, suites):
        ...
        suite_info += (
            "Cipher Strength: " + str(suites["cipherStrength"]) + "\n"
        )
