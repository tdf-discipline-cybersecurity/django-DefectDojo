...
class ContrastParser(Parser):
    ...
    def get_findings(self, filename, test):
        ...
        for row in reader:
            ...
            # Name,Severity,Status,Number of Events,First Seen,Last
            # Seen,Application Name,Application ID,Application Code,CWE
            # ID,Request Method,Request Port,Request Protocol,Request
            # Version,Request URI,Request Qs,Request Body
            ...
            if dupe_key in dupes:
                ...
                dupes[
                    dupe_key
                ].unique_id_from_tool = None  # there is no uniq finding now
            else:
    ...
    def format_description(self, row):
        ...
        description = (
            description
            + "**Request URI**: "
            + str(row.get("Request URI"))
            + "\n"
        )
        description = (
            description + "**Rule Name:** " + row.get("Rule Name") + "\n"
        )
        description = (
            description
            + "**Vulnerability ID:** "
